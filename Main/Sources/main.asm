...
;  Initialize LCD
lcd_init:
;The following changes to work with I2C LCD - <*DCV-161014*>
	      ;ldaa	#$ff
	      ;staa	DDRK		              ; port K = output
	      ldab #$1f                   ;100KHz, use $45 for 400
	      jsr I2C_Init
;end - <*DCV-161014*>
...
;Original LCD code (Dragon board LCD connected to port K) <*DCV-161014*>
...
;<*DCV-161014*>
;Added LCD code to write to I2C LCD
;D.Vogel 10/14/16
; Sunfounder I2C LCD pinout:
;   data pins mapped to P4-P7 of the PCF8574 expander
;   P0 mapped to LCD RS line
;   P1 mapped to LCD R/W line (always low for this app)
;   P2 mapped to LCD E line
;   P3 mapped to LCD background line (I think, always high)
;   
write_instr_nibbleLCD:
        anda    #$F0      ;Data in high nybble
        oraa #$0c         ; E = 1; RS = 0
        jsr I2C_writeLCD
        ldy     #10
winl    dey
...
write_instr_byte:
        psha
        jsr     write_instr_nibbleLCD  ;<*DCV-161014*>
        pula
        asla
        asla
        asla
...
write_data_byte:
        psha
        jsr     write_data_nibbleLCD  ;<*DCV-161014*>
        pula
        asla
        asla
        asla
...
;<*DCV-161014*>
;Added to add support for I2C LCD display
;D.Vogel 10/14/16    
                                                  
I2C_Init:
  stab IBFD
  movb #2, IBAD         ;set save address to 2 - pretty sure superfluous
  movb #0, IBCR         ;Disable I2C, does reset, just in case
  movb #$80, IBCR       ;Enable I2C, no IRQs
  rts


I2C_writeLCD:
  brset IBSR, #$20, *   ;Wait for bus not busy  
  pshb                  ;Can't trash B
  ldab #$7E             ;Device Id for LCD  SunFounder #$4E/Ebay#$7E
  bclr IBCR, $08        ;Acknowledge received bytes
  bset IBCR, $30        ;Master and Tx modes
  stab IBDR             ;Tx device ID
  brclr IBSR, #$02, *   ;Wait for irq flag
  bset IBSR, #$02       ;Reset irq flag
  brset IBSR, #$01, *   ;Wait for ack
